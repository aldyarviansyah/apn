        ...
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-md-6">
                    ...
                    <form class="apn-form" action="{{ route('piers.store') }}" method="post">
                        @csrf

                        <div class="form-section mb-40">
                            <div class="form-group">
                                <label>Pier number</label>
                                <input type="number" class="form-control gmod @error('number') is-invalid @enderror" name="number" value="{{ old('number') }}">
                            ...
                            <div class="form-group">
                                ...
                                <select class="custom-select @error('pier_category_id') is-invalid @enderror" name="pier_category_id">
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" {{ old('pier_category_id')==$category->id?'selected':''  }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select class="custom-select @error('section') is-invalid @enderror" name="section">
                                    <option value="west" {{ old('section')=='west'?'selected':'' }}>West</option>
                                    <option value="east" {{ old('section')=='east'?'selected':'' }}>East</option>
                                    <option value="center" {{ old('section')=='center'?'selected':'' }}>Center</option>
                                </select>
                                @error('section')
                                <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                @enderror
