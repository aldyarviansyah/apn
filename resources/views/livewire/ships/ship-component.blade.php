<div>
    <div id="ui-specifics">
        ...
        @endif
        @include('livewire.ships.includes.detailAndSearch')
    </div>
    <section id="content">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    ...
                    @if($ships->total() > 0)
                        <div class="table-meta" wire:loading.class="d-none" wire:target="previousPage,nextPage">
                            Viewing <span class="black">{{ $ships->lastItem() }}</span> <span>out of</span> <span class="black">{{ $ships->total() }}</span> {{ $title }}
                            @if($keyword||$column||$category)
                                <a href="{{ route('ships') }}" class="table-meta-desc">
                    ...
                    @foreach($ships as $ship)
                        <div class="data-row open-view" wire:click="showDetail({{ $ship->id }})" wire:key="{{$ship->id}}">
                            <div class="left">
                                <div class="upper">
                                    <div class="inner">{{ $ship->name }}</div>
                                </div>
                            </div>
                            <div class="right">
                    ...
                    @if($ships->total() > 0)
                        <div class="table-meta-bottom" wire:loading.class="d-none" wire:target="previousPage,nextPage">
                            Viewing <span class="black">{{ $ships->lastItem() }}</span> out of <span class="black">{{ $ships->total() }}</span> {{ $title }}
                        </div>
                        <div class="table-meta w-100 block" wire:loading wire:target="previousPage,nextPage">
                            <span class="black">Loading...</span>
                        </div>
                    ...
                    {{ $ships->links('vendor.pagination.hms-livewire') }}
